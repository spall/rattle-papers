...
Speculation, a computation running based on an unverified assumption, has also been used in the area of distributed systems. \cite{tapus2006distributed} explores the use of
speculation as a form of distributed transactions for use in distributed systems.  As with other
work using speculative execution, if the assumption which led to a speculation is proven to be
invalid, the computation is aborted and the program is rolled back.  If the assumption was valid,
then the results of the speculated computation are committed.  As part of this thesis, components of a distributed filesystem supporting process speculation were implemented.  And, \cite{nightingale2005speculative} showed that adding support for speculation to the Linux kernel could improve the
performance of existing distributed file systems, by allowing multiple file system operations to
occur concurrently while maintaining consistency.
