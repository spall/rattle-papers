...
For benchmarks, the first four (\S\ref{sec:eval:introduction}-\S\ref{sec:eval:redis}) were run on a 4 core Intel i7-4790 3.6GHz CPU (16Gb RAM, 7200 RPM rotating hard drive). The remaining benchmarks were run on a 32 core Intel Xeon E7-4830 2.13GHz (64Gb RAM).
...
\textbf{Overhead} The overhead inherent in \Rattle is greater than that of \Make, as it hashes files, traces command execution, computes potential hazards, figures out what to speculate and writes to a shared cloud store (in this section we use a locally mounted shared drive). To measure the overhead, and prove the other claims in this section, we created a very simple pair of files, \texttt{main.c} and \texttt{util.c}, where \texttt{main.c} calls \texttt{printf} using a constant returned by a function in \texttt{util.c}. We then measured the time to do: (1) An initial build from a clean checkout;
