...
\begin{verbatim}
import Development.Rattle, System.FilePath, System.FilePattern
...
\item On \textbf{Windows} we can't trace 64bit programs spawned by 32bit programs, but such a pattern is rare (most binaries are now 64bit), often easily remedied (switch to the 64bit version) and we have the code to fix it if necessary (although the integration is likely difficult).
\end{itemize}
