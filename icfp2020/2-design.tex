...
If a build using speculative execution causes a hazard, it is possible that the hazard is entirely an artefact of speculation. However, there are other possible approaches.

\begin{description}
\item[Restart without speculation] If a hazard occurs, and speculation was involved, we can can abort the build retry without speculation.
\item[Raise a hazard] If the hazard occurs due to two non-speculated commands run in the order they were requested, then the build is incorrect, and rerunning will cause the hazard. That hazard can be raised immediately.
...
Given our model of treating the user script as a black box, rewinding is not an option. The remaining approaches are, but we discuss the precise circumstances in each is safe in \S\ref{sec:proof:classify_hazard}.
