...
\begin{itemize}
\item In \S\ref{sec:rattle} we describe how tracing can turn a shell script into a build system, while skipping dependencies and using a cloud store. To make this transformation requires \emph{tracing}, which we describe in \S\ref{sec:tracing}. % \item In \S\ref{sec:speculation} we add parallelism, which requires speculation.
