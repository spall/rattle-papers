...
% a new build script was generated for 47174f51, which meant I ran sh autogen.sh && ./configure again
% before running the rattle build script in case important files needed to be generated
% Looking at the diff for this commit between rattle and make, I see Make running what appear to be
% automake etc commands, so I am inclined to ignore those commands.
% I also see Rattle re-building tmux, which make does not appear to do; it did this becuase
% tmux.o changed

% not great for rattle

% it seems this probably happened because a version number changed; the version number is a part
% of the gcc commands run, which rattle saw as a new command, and so it ran the commands producing
% new object files.... and therefore a new executable.
