...
On Windows, enabling the default anti-virus (Windows Defender) has a significant impact on the result, increasing the \Make baseline by 11\% and the final time by 29\%. These results were collected with the anti-virus disabled.
 ...
 In contrast, on Linux, the total overhead is only 7\%, of which nearly all (5\%) comes from tracing.
...
From the chart, we can see that on average \Rattle with 4 threads took longer than single threaded \Rattle, for commits 2, 12, and 29 in the chart.  \emph{read-write hazards} occurred when \Rattle was building these commits, forcing \Rattle to restart the build with no speculation.  For each of these commits the \Rattle build script changed, so hazards occurred when \Rattle speculated commands from the previous script then ran the changed commands from the new script.
