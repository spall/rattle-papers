...
\subsection{Writing \Rattle builds}

SARAH: FIGURING OUT IF THIS SECTION IS REQUIRED, SO VERY DRAFT. DOES IT HELP SELL THE STORY? CAN FINISH IT OFF TONIGHT.

In order to check that writing build scripts with \Rattle is an easy thing to do we tried writing two projects in \Rattle -- namely \Fsatrace and a program to fetch Haskell dependencies and install them.

\subsubsection{Implementing \Fsatrace}

We implemented FSATrace from scratch. We did all the compilations with explicit parallelism, e.g. \texttt{forP}. Using Haskell, it was way easier to describe the abstraction so the result is much shorter and clearer. It ended up being N lines long.

\subsubsection{Reimplementing Stack}
