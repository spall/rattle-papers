...
The build both seems to be using a form of tracing to keep track of accurate dependencies as well as tracking whether or not the build itself has changed.  \Rattle would make it unnecessary for the build to explicitly do this, since it internally traces and tracks the dependencies of all commands run, and doesn't need to worry about when a build changes.
