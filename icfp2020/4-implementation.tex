...
Given the above API, combined with the choice to treat the control logic as opaque, it is possible to write wrappers that expose \Rattle in new ways. For example, to literally run a series of commands, it is possible to write the \Rattle program:
...
Here we get a single command line argument, read it as a file, then run each command sequentially using \texttt{forM\_}. We use this script in our evaluation in \S\ref{sec:evaluation}.

An alternative API could be provided by opening up a socket, and allowing a \Rattle server to take command invocations through that socket. Such an API would allow writing \Rattle scripts to be written in other languages, making use of the existing \Rattle implementation. While such a design should be easy, we have not yet implemented it.

\subsection{Hash forwarding}
\label{sec:forward_hashes}
% SS I understand hash forwarding but i found this section confusing
...
\item \Rattle can go wrong if a speculated command writes to a source file. We haven't had that happen in practice, but dividing files into inputs and outputs would be perfectly reasonable -- typically the inputs are checked into version control so that information is trivially available.
\end{itemize}

It is also important to note that we don't store a trace as soon as it is produced, because a future hazard involving it might occur, since we only know if traces are valid at the end.
...
\item[File system tracing] An alternative approach on Linux would be to implement a file system that reported which files were accessed. We are aware of one such implementation, \tracedfs \cite{tracedfs}, which is unfortunately not yet complete. Such an approach would track all accesses, but may require privileges to mount a file system.
  % SS TODO add something about Facebook's Watchman file watching service
