...
\proof The proof from \S\ref{sec:argument} shows that any script with no hazards will result in a fixed point. To prove any fixed point is identical, we define the inputs as the set of files that commands read from, but no command writes to, namely:

\[
  \bigcup_{c \in D} c_r \setminus \bigcup_{c \in D} c_w
\]

We can prove the stronger claim, that for any filesystem state where all inputs have identical contents, there is only one fixed point. For each command, it can only read from files that are in the inputs or files that are the outputs of other commands. Taking these two cases separately:
