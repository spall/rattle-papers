...
\claim A build with deterministic control logic that completes with no hazards has reached a fixed point.  % meaning if re-run, nothing would change
...
As a practical matter, if a command is part of the commands that cause a hazard, it is a good candidate for not repeating.

\subsubsection{Raise a hazard}

Importantly, if there is no speculation (including reordering and parallelism), and the build raises a hazard, the hazard can be raised immediately. This property is essential as a base-case for restarting with no speculation.

If two required commands cause a write-write hazard, and there have been no speculative-write-read hazards, then the same hazard will be reraised if the command restarts. For write-write, this inevitability is a consequence of \S\ref{sec:proof:reorder}, as the same writes will occur.

If two required commands cause a read-write hazard, and the relative order they were run in matches their required ordering, then the same hazard will be reraised if the command restarts because the ordering will remain the same. However, if the commands weren't in an order matching their required ordering, it is possible the write will come first avoiding the hazard.

\subsubsection{Continue}

If the only effects of a hazard are on speculated commands, and those commands have no subsequent effect on the required commands, it is possible to continue the build. However, it is important that the commands have no subsequent effect, even on required commands that have not yet been executed, or even revealed to the build system, so continuing may still fail in the future, even if it is permissible now. To determine whether a build can continue, it is important to note where the effects of a hazard lie:

\begin{enumerate}
\item A write-write hazard has an effect on both commands that wrote to the file in question.
\item A read-write hazard has an effect on the command that read, but not on that which wrote.
\item A speculative-write-read hazard has an effect on the reader, but not the writer. However, for this hazard, the reader is always a required command, so this hazard can never be continued.
\end{enumerate}

We say that any command directly effected by any hazard is poisoned. Furthermore, any command which reads any files written by a poisoned command is also poisoned. If any required file is poisoned the build cannot continue.
