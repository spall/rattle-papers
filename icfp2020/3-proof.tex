...
\item[Speculative-write-read hazard] A build has a \emph{speculative-write-read hazard} if a file is written to by a command which is speculated, then read from by a command in the build script before the speculated command has been encountered in the build script.
\end{description}
...
\claim If a set of commands $C$ is run, and no hazards arise, then every command within $C$ has reached a fixed point.
...
\claim Given a script with no hazards when executed, speculating unnecessary commands that do not lead to hazards will not have an effect on the build's inputs or outputs.
...
\claim If a sequence of commands leads to a hazard, any additional speculative commands or reordering will still cause a hazard.
...
If the hazard only involves speculated commands, and those commands have no subsequent affect on required commands, it is possible to continue the build. However, it is important that the commands have no subsequent effect, even on required commands that have not yet been executed, or even revealed to the build system. Consequently, if a build continues after hazards, it may fail even if there are no future hazards. To determine whether a build can continue, we can define which commands are ``poisoned'':

%% SS gonna come back to this

\begin{enumerate}
\item A write-write hazard poisons commands which wrote to the file in question.
\item A read-write hazard poisons the command which read, but not one that wrote.
\item A speculative-write-read hazard poisons the command which read, but not that which wrote.
\item A command which reads any files written by a poisoned command is itself poisoned.
\end{enumerate}

If any required command is poisoned (either now or by the end of the build), then the build cannot continue.
