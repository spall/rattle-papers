...
% SS I think I edited this sentence to be like this, but I still dont like how it sounds
The design of \Rattle relies on taking a sequence of commands and rathern than running them all sequentially, instead running them in different orders and with speculation. In this section we prove that the manipulations we perform are safe with respect to the reference semantics. Throughout these proofs we assume the commands themselves are pure functions from the inputs to their outputs (as reported by tracing).
...
Because the commands in the build will have not changed, it follows from the proof in \S\ref{sec:argument} that the commands will not changes the values of any files, therefore, the build is at a fixed point.
...
\proof The proof from \S\ref{sec:argument} shows that any script with no hazards will result in a fixed point. To prove equivalency we define the inputs as the set of files that commands read from, but no command writes to, namely:

\[
  \bigcup_{c \in D} c_r \setminus \bigcup_{c \in D} c_w
\]

% is this supposed to say something instead of ``file states''? 
For any file states where the inputs are equivalent, there is only one fixed point. The commands only read from files that are in the inputs, or files that are written by another command. % mean first command in build right; not just first command for any output
The outputs of the first command in the build can only have been influenced by the inputs, since if it read any other writes, there would be a read-write hazard
, so that first output must be a consequence of only inputs. The second command can only look at the inputs and the files written by the first command, all of which are determined by the inputs. By induction we can see that all outputs must be influenced only by the inputs.
